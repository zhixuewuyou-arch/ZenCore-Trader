...
def get_pro_market_data(code):
    try:
        # 1. 获取更多历史数据以确保均线准确
        df = ak.stock_zh_a_hist(symbol=code, period="daily", adjust="qfq").tail(200)
        ...
        try:
            ...
            current_price = spot_df[spot_df['代码'] == code]['最新价'].values[0]
        except:
            current_price = df['收盘'].iloc[-1]
            
        return df.tail(100), current_price # 仅展示最近100天
    except:
        return None, None

# --- 5. 主界面 ---
st.title("ZenCore AI 操盘手 v1.3")
...
with tabs[0]:
    ...
    for code, info in st.session_state.portfolio.items():
        _, price = get_pro_market_data(code)
        if price:
...
with tabs[1]:
    ...
    df, price = get_pro_market_data(sel_code)
...
with tabs[2]:
    if st.button("启动大数据审计"):
        ...
        news = ak.stock_news_em(symbol=sel_code).head(10)['新闻标题'].tolist()
        response = model.generate_content(f"标的：{sel_code}, 现价：{price}, 新闻：{news}")
